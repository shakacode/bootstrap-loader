...
test('buildExtractStylesLoader throws error if ary doesnt have "style-" at ary[0]', assert => {
  assert.throws(
    () => {
      buildExtractStylesLoader(['some-loader', 'some-other-loader']);
    },
    Error(`
...
test('buildExtractStylesLoader runs as expected', assert => {
  assert.equals(
    buildExtractStylesLoader(['style-loader', 'url-loader', 'css-loader']),
    `${fs.realpathSync(
      path.join(__dirname, '../../../node_modules/extract-text-webpack-plugin'),
    )}/loader.js?{"omit":1,"remove":true}!style-loader!url-loader!css-loader!`,
  );
  assert.equals(
    buildExtractStylesLoader(['isomorphic-style-loader', 'url-loader', 'css-loader']),
    `${fs.realpathSync(
      path.join(__dirname, '../../../node_modules/extract-text-webpack-plugin'),
